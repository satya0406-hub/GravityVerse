  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Firestore save failed or bypassed:", e);
      ...
      try {
        ...
        const timeoutId = setTimeout(() => controller.abort(), 6000); // 6 seconds timeout fallback
        ...
        if (response.ok) {
          ...
          console.warn("Backend email dispatch returned an error status:", responseData.error || response.statusText);
        }
      } catch (emailErr) {
        console.warn("Backend email dispatch timed out or was unreachable:", emailErr);
      }

      // We succeed as long as we successfully stored it in Firestore or successfully emailed it
      if (!firestoreSaved && !emailDispatched) {
        throw new Error('Both database backup storage and email dispatch failed. Please check your internet connection.');
      }
