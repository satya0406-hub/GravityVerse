...
async function startServer() {
  ...
  // CORS Middleware for Cross-Origin Requests (e.g. from GitHub Pages to Render Backend)
  ...
  // Professional Neural Request Logger & Path Normalizer
  app.use((req, res, next) => {
    const start = Date.now();
    const originalUrl = req.url;
    
    // Normalize path: handle optional base prefixes for various deployment environments
    const prefixes = ['/PRINCE_STAR/', '/PRINCE-STAR/', '/GRAVITYVERSE/', '/GRAVITY-VERSE/', '/GravityVerse/', '/Gravity-Verse/', '/ais-dev/', '/ais-pre/']; 
    for (const prefix of prefixes) {
      if (req.url.toUpperCase().startsWith(prefix.toUpperCase())) {
        req.url = '/' + req.url.slice(prefix.length).replace(/\/+/g, '/');
        break;
      }
    }
    ...
    res.on('finish', () => {
      ...
      if (req.url.includes('/api/')) {
        ...
        // Skip logging successful source file requests to keep console clean
      }
    });
    next();
  });
  ...
  app.post("/api/contact", async (req, res) => {
    ...
    const recipient = process.env.CONTACT_RECEIVER_EMAIL || "support@gravityverse.in";
    ...
    console.log("[SMTP DEBUG] Environment check:", {
      CONTACT_RECEIVER_EMAIL: recipient,
      SMTP_HOST: smtpHost,
      SMTP_PORT: smtpPort,
      SMTP_USER_defined: !!smtpUser,
      SMTP_PASS_defined: !!smtpPass,
    });
    ...
    try {
      ...
      if (smtpUser && smtpPass) {
        // User configured real SMTP credentials
        try {
          console.log("[SMTP Status] Connecting to custom mail gateway...");
          ...
          // Fast verify to fail early if credentials require activation or are incorrect
          await customTransporter.verify();
          console.log("[SMTP Status] Custom mail gateway verified successfully!");
          transporter = customTransporter;
        } catch (verifyError: any) {
          console.log("[SMTP Status] Custom mail gateway is pending activation or requires configuration. Routing via sandbox.");
          smtpAuthError = "Brevo SMTP connection pending activation (SMTP Relay account not fully active)";
          transporter = null;
        }
      }

      if (!transporter) {
        // Dynamic Ethereal account creation for stellar testing preview
        console.log("[SMTP Status] Sandbox mailer gateway initialized.");
      ...
      const mailOptions = {
        from: isEthereal ? `"${name}" <${email}>` : `"${name} (via GravityVerse)" <support@gravityverse.in>`,
        to: recipient,
        replyTo: email,
...
`,
      ...
      if (isEthereal) {
        ...
        console.log(`[SMTP] Ethereal inbox URL preview: ${testUrl}`);
      }

      return res.json({
        success: true,
        message: smtpAuthError 
          ? "Message received! Custom SMTP connection failed verification, so the message was safely rerouted via developer sandbox."
          : "Message successfully processed and email transmitted.",
        messageId: info.messageId,
        isDemoMailer: isEthereal,
        demoInboxUrl: testUrl,
        recipient: recipient,
        smtpAuthError: smtpAuthError
      });
    } catch (err: any) {
      console.log("[SMTP Status] Dispatch issue occurred during mailer route:", err.message);
      return res.status(500).json({
        success: false,
        error: "Message delivery pending system confirmation: " + err.message
      });
    }
  });

  app.post(["/api/chat", "/api/chat/"], async (req, res) => {
    const { prompt, history, systemInstruction, customApiKey, system_instruction, custom_api_key } = req.body;
